...
function filterProducts(criteria) {
    return products.filter(product => {
        return Object.keys(criteria).every(key => {
            if (key === "priceRange") {
                return product.price >= criteria[key][0] && product.price <= criteria[key][1];
            }
            if (key === "review") {
                return product.review >= criteria[key];
            }
            if (key === "variants") {
                return product.variants.includes(criteria[key]);
            }
...
// Example usage:
const filteredByPrice = filterProducts({ priceRange: [100, 500] });
const filteredByReview = filterProducts({ review: 4.5 });
const filteredByVariant = filterProducts({ variants: "Black" });
...
console.log("Filtered by Review:", filteredByReview);

console.log("--------------------------------------------");

console.log("Filtered by Variant:", filteredByVariant);
...
console.log("--------------------------------------------");
  ...
  // Sorting function
  function sortProducts(criteria, order = "ascending") {
    return products.sort((a, b) => {
      let valueA, valueB;
  
      // Determine the sorting value based on criteria
      switch (criteria) {
        case "price":
          valueA = a.price;
          valueB = b.price;
          break;
        case "discount":
          valueA = a.compare_at_price - a.price;
          valueB = b.compare_at_price - b.price;
          break;
        case "variants":
  ...
  // Example usage
  const sortedByPriceAsc = sortProducts("price", "ascending");
  const sortedByDiscountDesc = sortProducts("discount", "descending");
  const sortedByVariantsAsc = sortProducts("variants", "ascending");
  const sortedByReviewDesc = sortProducts("review", "descending");
  
  console.log("Sorted by Price (Ascending):", sortedByPriceAsc);

  console.log("--------------------------------------------");

  console.log("Sorted by Discount (Descending):", sortedByDiscountDesc);

  console.log("--------------------------------------------");

  console.log("Sorted by Variants (Ascending):", sortedByVariantsAsc);

  console.log("--------------------------------------------");

  console.log("Sorted by Review (Descending):", sortedByReviewDesc);
